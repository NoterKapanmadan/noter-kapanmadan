...
import { sendHistory, isAuthenticated, getAccountID, getBrands, getModels, getRatings } from "@/app/actions";
...
export default async function AdPage({ params }) {
  ...
  const ratings = await getRatings(ad.account_id);
  
  return (
    <div className="min-h-screen bg-gray-50">
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <AdView ratings={ratings} ad={ad} isAuth={isAuth} currentUserID={account_id} brands={brands} defaultBrandModels={models} metricsData={metricsData}/>
      </main>
    </div>
  );
}